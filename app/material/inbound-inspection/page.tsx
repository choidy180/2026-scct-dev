...
// --- Styled Components (Loading Screen) ---

const LoadingWrapper = styled.div<{ $isFadingOut: boolean }>`
  position: fixed;
  top: 0;
  left: 0;
  width: 100vw;
...
const StatusText = styled.div`
  ...
  color: #64748b;
  font-weight: 500;
  min-height: 1.2em; /* 텍스트 변경 시 레이아웃 점프 방지 */
  display: flex;
...
export default function SmartFactoryDashboard() {
  ...
  useEffect(() => {
    ...
    if (loading) {
      interval = setInterval(() => {
        setProgress((prev) => {
          // 진행률에 따라 속도 및 텍스트 변화
          let increment = Math.random() * 5; // 기본 증가량
          
          if (prev < 30) {
            setStatusText('시스템 모듈 로드 중...');
            increment = Math.random() * 8; // 초반엔 빠름
          } else if (prev < 60) {
            setStatusText('보안 프로토콜 연결 중...');
            increment = Math.random() * 3; // 중간엔 조금 느림 (연결 대기 느낌)
          } else if (prev < 85) {
            setStatusText('AI 비전 모델 동기화 중...');
            increment = Math.random() * 4;
          } else {
            setStatusText('대시보드 구성 완료...');
            increment = Math.random() * 10; // 마무리 쓩
          }

          const nextProgress = prev + increment;
          if (nextProgress >= 100) {
            clearInterval(interval);
            // 100% 도달 후 잠시 대기했다가 페이드아웃
            setTimeout(() => setIsFadingOut(true), 500);
            setTimeout(() => setLoading(false), 1300); // 페이드아웃 시간 고려
            return 100;
          }
          return nextProgress;
        });
  ...
  return (
    <>
      ...
      {/* --- Main Dashboard --- */}
      {/* 로딩이 완전히 끝나기 전에는 렌더링은 하되 보이지 않게 처리하거나, 아예 분리할 수도 있습니다. 
          여기서는 자연스러운 등장을 위해 isFadingOut이 true가 될 때(거의 다 됐을 때) 마운트된 상태로 둡니다. */}
      {!loading && (
