...
import { 
  useGLTF, Stage, OrbitControls, Html, useProgress, Center, Resize, Environment, Bounds 
} from "@react-three/drei";
import { Box as LucideBox, Activity, Thermometer, Cpu, Scan, BarChart3, TrendingUp, Zap, Layers, AlertTriangle } from "lucide-react";
...
class ModelErrorBoundary extends React.Component<{ fallback: React.ReactNode, children: React.ReactNode }, { hasError: boolean }> {
  ...
  componentDidCatch(error: any, errorInfo: any) { console.error("3D Model Loading Failed:", error); }
  ...
  return (
    <group>
      ...
      <Html position={[0, 1.2, 0]} center>
        <div style={{ color: 'red', background: 'rgba(0,0,0,0.8)', padding: '4px 8px', borderRadius: '4px', border: '1px solid red', fontSize: '10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><AlertTriangle size={10} /> Not Found</div>
...
const Fill = styled.div<{ $p: number }>` height: 100%; width: ${props => props.$p}%; background: linear-gradient(90deg, #38bdf8, #818cf8); transition: width 0.2s ease; `;
function Loader({ onFinished }: { onFinished: () => void }) {
  ...
  useEffect(() => {
    let frame: number;
    const loop = () => {
      setVal(v => { const target = active ? progress : 100; const next = v + (target - v) * 0.1; if(Math.abs(target - next) < 0.1) return target; return next; });
      frame = requestAnimationFrame(loop);
    };
    loop(); return () => cancelAnimationFrame(frame);
  }, [active, progress]);
  useEffect(() => { if(val > 99.5) { setTimeout(() => { setDone(true); onFinished(); }, 600); } }, [val, onFinished]);
  if(done) return null;
  return (<LoaderOverlay><LoadingBarContainer><LoadingText><span>System Loading</span><strong>{val.toFixed(0)}%</strong></LoadingText><Track><Fill $p={val} /></Track></LoadingBarContainer></LoaderOverlay>);
}
...
function StatusPopup({ data, position }: { data: any, position: [number, number, number] }) {
  ...
  return (
    <Html position={position} center zIndexRange={[999999, 0]} occlude={false} style={{ pointerEvents: 'none', width: 0, height: 0, overflow: 'visible' }}>
...
function InteractiveJigModel({ url }: { url: string }) {
  ...
  const handlePointerOver = (e: any) => {
    ...
    if (mesh.isMesh) {
      ...
      const center = new THREE.Vector3(); box.getCenter(center);
      setHoveredData({
        id: mesh.uuid, name: mesh.userData.name || "Unknown Unit",
  ...
  return (
    // [수정 3] 오브젝트 위치 높이기: Y축 0.2 -> 0.5로 변경
        ...
        <ViewerContainer $visible={loaded}>
          ...
          >
            ...
            <Suspense fallback={null}>
              {/* [수정 1] 크기 키우기: margin을 1.2에서 0.92로 줄여서 꽉 차게 보이게 함 */}
