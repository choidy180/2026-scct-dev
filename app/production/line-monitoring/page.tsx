...
// -----------------------------------------------------------------------------
// [Styled Components - One Property Per Line]
...
const TrendBadge = styled.div<{ $isUp: boolean }>`
  font-size: 12px;
  font-weight: 700;
  color: ${(p) => (p.$isUp ? THEME.primary : THEME.danger)};
  display: flex;
  align-items: center;
  gap: 4px;
`;
...
const SpinnerRing = styled.div<{ $size: number, $color: string, $speed: number, $reverse?: boolean }>`
  ...
  border-left-color: ${(p) => p.$color + '60'};
...
// [최적화] CyberLoader: 0~100까지 부드럽게 2.5초간 진행
...
// [최적화] TransitionLoader: 화면 전환용 로더
...
// [최적화] CustomTooltip: 타입 안정성 확보 및 메모이제이션
  ...
  useEffect(() => {
    ...
    scene.traverse((child) => {
      if ((child as THREE.Mesh).isMesh) {
        ...
        // [에러 수정] Material Type Assertion
        if (mesh.material) {
          const oldMat = Array.isArray(mesh.material) ? mesh.material[0] : mesh.material;
          // color 속성이 있는지 확인 후 접근하거나 StandardMaterial로 단언
    ...
    <PageContainer>
      <MainContent>
        ...
        <ViewerContainer $visible={initialLoaded}>
          ...
          <Panels hoveredInfo={hoveredInfo} errorUnits={errorUnits} />

          <Canvas
            dpr={[1, 1.5]} // [최적화] 4K 스크린 대응: 2.0 -> 1.5로 제한
